...
export class DashboardService {
  ...
  updateCliente(id: number | string, updatedCliente) {
    return this.http.put(
      `${environment.API_URL}/dashboard/datos-personales/${id}`,
      updatedCliente
    );
  }
  ...
  guardarDirecion(direccion) {
    const headers = new HttpHeaders({ 'content-type': 'application/json' });
    return this.http.post(
      `${environment.API_URL}/dashboard/datos-personales/addDireccion`,
      direccion,
      { headers: headers }
    );
  }
  ...
  getDomiciliosCleinte(id: number) {
    return this.http.get(
      `${environment.API_URL}/dashboard/datos-personales/getDireccion/${id}`
    );
  }
}
